...
# Complete fix for recurring file corruption issue
# This script completely recreates the corrupted files and applies all fixes

echo "=== COMPLETE SYNTAX FIX SCRIPT ==="

# Step 1: Remove corrupted file
rm -f client/src/components/case-entry-form.tsx

# Step 2: Create fresh working file with heredoc
...
# Step 3: Fix server storage import
sed -i 's/import { eq, and, or, desc, asc }/import { eq, and, or, desc, asc, ilike }/' server/storage.ts
echo "✓ Fixed server storage import"

# Step 4: Verify files are correct
echo "=== Verifying case-entry-form.tsx ==="
head -3 client/src/components/case-entry-form.tsx

echo "=== Verifying storage.ts import ==="
head -10 server/storage.ts | grep -E "ilike|import.*drizzle"
